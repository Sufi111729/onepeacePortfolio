...
export function AdminLoginForm() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (active && data.session) router.replace('/admin/messages');
    });
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Network error. Please check your Supabase settings.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-ink px-5 py-12 text-text sm:px-6 lg:px-8">
      ...
      <section className="relative mx-auto flex min-h-[calc(100vh-6rem)] max-w-md items-center">
        <form onSubmit={handleLogin} className="w-full border border-line bg-surface p-6 shadow-clean sm:p-8" aria-label="Admin login form">
          ...
          <p className="mt-4 text-sm leading-6 text-muted">Sign in to manage contact messages securely.</p>

          <div className="mt-8 grid gap-5">
            <label className="form-field">
              <span>Email</span>
              <input type="email" autoComplete="email" placeholder="admin@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required />
            </label>
            <label className="form-field">
              <span>Password</span>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Your password" value={password} onChange={(event) => setPassword(event.target.value)} className="pr-14" required />
                <button
                  type="button"
            ...
            <Link href="/" className="inline-flex items-center justify-center gap-2 text-sm font-semibold text-muted transition hover:text-accent focus-ring">
